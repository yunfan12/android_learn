package com.wdys.android_learn.cusview;
...
public class ImageCycleView extends LinearLayout {
	 ...
	 * 图片轮播视图
	 */
	public CycleViewPager mBannerPager = null;
	 ...
	 */
	public void setImageResources(ArrayList<ADInfo> infoList, ImageCycleViewListener imageCycleViewListener,String Tag) {
		urls = new String[infoList.size()];
		for (int i = 0; i < infoList.size(); i++) {
			urls[i] = infoList.get(i).getUrl();
		}
		// 清除所有子视图
		...
		for (int i = 0; i < imageCount; i++) {
			...
			if (i == 0) {
				mImageViews[i].setBackgroundResource(R.drawable.true_dot);
			} else {
				mImageViews[i].setBackgroundResource(R.drawable.false_dot);
			}
			mGroup.addView(mImageViews[i]);
		}
		mAdvAdapter = new ImageCycleAdapter(mContext, infoList, imageCycleViewListener,Tag);
	...
	private final class GuidePageChangeListener implements OnPageChangeListener {
		...
		public void onPageSelected(int index) {
			...
			mImageIndex = index;
			index -= 1;
			mImageViews[index].setBackgroundResource(R.drawable.true_dot);
	...
	private class ImageCycleAdapter extends PagerAdapter {
        ...
        private String Tag;
		public ImageCycleAdapter(Context context, ArrayList<ADInfo> adList, ImageCycleViewListener imageCycleViewListener,String Tag) {
			mContext = context;
			mAdList = adList;
            this.Tag=Tag;
		...
		public Object instantiateItem(ViewGroup container, final int position) {
			...
			if (mImageViewCacheList.isEmpty()) {
                ...
                if(Tag.equals("HomePageFragment")){
